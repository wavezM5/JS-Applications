    ...
    <section id="search">
        ...
        <div id="search-container">
            <ul class="card-wrapper">
                ...
                ${shoesList.length === 0 ? html`<h2>There are no results found.</h2>` : shoesList.map(shoe =>
                shoeTemplate(shoe, userId))}
            </ul>
        </div>
    </section>
`;
...
export async function searchPage(ctx) {
    ...
    const shoesList = brand == undefined ? [] : await getShoeByQuery(brand);
