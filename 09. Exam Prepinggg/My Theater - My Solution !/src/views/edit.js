...
<section id="editPage">
    <form @submit=${onSubmit} class="theater-form">
        ...
        <div>
            ...
            <textarea id="description" name="description" placeholder="Description">${movie.description}</textarea>
        </div>
        <div>
...
export async function editView(ctx) {
    ...
    async function onSubmit(event) {
        ...
        if (checkEmptyFields(title, date, author, description, imageUrl)) {
            return alert("All fields must not be empty!!!!!");
        }

        await updateRecord(id, title, date, author, description, imageUrl);
